...
class ScrambleRepository {
  ...
  String _get2by2Scramble() {
    ...
    var temp = -1;
    final lit = [["R", "R'", "R2"], ["F", "F'", "F2"], ["U", "U'", "U2"]];
    while (true) {
  ...
  String _get3by3Scramble() {
    ...
    while (true) {
      ...
      if (scrambleList.length > 2 && scrambleList[scrambleList.length - 2] == lit[i][j] || banned.contains(i)) {
        continue;
      }
      scrambleList.add(lit[i][j]);
  ...
  String _getClockScramble() {
    final scramble = ["UR", "num", "sign", "DR", "num", "sign", "DL", "num", "sign", "UL", "num", "sign",  "U",
